        ...
        <div class="container-fluid my-3  ">
            <div class="row justify-content-center">
                <div class="card-create-project pt-4 my-3 mx-5 px-5">
                  ...
                  <p id="pcreateProject">dashboard to manage all created skills</p>
                    @can('project_create')
                    <a type="button" class="rounded" href="{{ route('admin.skills.create') }}" style="text-decoration: none;" role="button"><div  class="action-main-create-button px-5 text-center py-3 rounded">Create New Skill</div></a>
                    @endcan
                </div>
            </div>
        </div>
...
@else 
<strong> There is no skills in your company </strong>
<hr>
@endif
...
<script>
  $('#search_button').on('click', function(){  
    ...
    queryString = $('#search-input').val(); 
        $.ajax({
            url: "{{ route('admin.skills.getSearchResult') }}",
            method: 'get',
            data: {
                "_token": "{{ csrf_token() }}",
            ...
            success: function(output){
                ...
                if(result != null){
                  $('#searchedSkillsTable').empty();
                  $('#sortedSkillsTable').hide(); 
                  $('#sortedSkillsTableSorted').hide();
                  $('#searchedSkillsTable').append(result[0]);
                  $('#searchedSkillsTable').show();
                  $('#loading').hide();
                }
            }
        });
